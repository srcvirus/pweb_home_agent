 ...
 */
boost::shared_ptr <User> UserCassandraController::getUser(const string& username)
{
	unsigned long tid = (unsigned long)pthread_self();

	string queryString = "select * from " + User::TABLE_NAME + " where " + User::COL_USER_NAME + " = '" + username + "';";
	...
	retObject = boost::shared_ptr <User> (new User(uname, password, email, fullname, location, affiliation));
...
{
        string queryString = "insert into " + User::TABLE_NAME + " (" + 
                                                        User::COL_USER_NAME + ", " + 
                                                        User::COL_PASSWORD + ", " + 
                                                        User::COL_EMAIL + ", " + 
                                                        User::COL_FULL_NAME + ", " + 
                                                        User::COL_LOCATION + ", " + 
                                                        User::COL_AFFILIATION + ") VALUES ('" + 
                                                                user->getUsername() + "', '" +
                                                                user->getPassword() + "', '" +
                                                                user->getEmail() + "', '" +
                                                                user->getFullname() + "', '" +
...
vector < boost::shared_ptr <User> > UserCassandraController::getAllUser()
{
	...
	while(rows.next())
	{
		...
		rows.get_string(User::COL_LOCATION, location);
		rows.get_string(User::COL_AFFILIATION, affiliation);
		boost::shared_ptr <User> user = boost::shared_ptr <User> (new User(uname, password, email, fullname, location, affiliation));
...
bool UserCassandraController::isUsernameAvailable(const string& username, string& errorCode)
{
	errorCode = "";
	unsigned long tid = (unsigned long)pthread_self();
	string queryString = "select * from " + User::TABLE_NAME + " where " + User::COL_USER_NAME + " = '" + username + "';";
...
bool UserCassandraController::authenticateUser(const string& username, const string& password, string& errorCode)
{
	errorCode = "";
	unsigned long tid = (unsigned long)pthread_self();
	string queryString = "select * from " + User::TABLE_NAME + " where " + User::COL_USER_NAME + " = '" + username + "' and " + User::COL_PASSWORD + " ='" + password + "';";
	printf("[DEBUG] [Thread 0x%lx] %s\n", tid, queryString.c_str());
...
int UserCassandraController::updateUser(const string& username, boost::unordered_map<string, string>& params)
{
	if(params.size() <= 0)
		return 0;
