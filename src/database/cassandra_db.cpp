...
int CassandraDBDriver::openConnection() {
  ...
  cqlBuilder->add_contact_point(
      boost::asio::ip::address::from_string(this->dbServerHostName),
      (unsigned short int)this->dbServerPort);
...
CassandraDBDriver::getDatabaseDriverObject() {
  if (!CassandraDBDriver::instanceExists) {
    CassandraDBDriver::databaseDriverObject = boost::shared_ptr<
        IDatabaseDriver<boost::shared_future<cql::cql_future_result_t> > >(
        new CassandraDBDriver());
    CassandraDBDriver::instanceExists = false;
  }
...
boost::shared_ptr<
    IDatabaseDriver<boost::shared_future<cql::cql_future_result_t> > >
CassandraDBDriver::getDatabaseDriverObject(const string &dbServerHostName,
                                           unsigned short int dbServerPort) {
  if (!CassandraDBDriver::instanceExists) {
    CassandraDBDriver::databaseDriverObject = boost::shared_ptr<
        IDatabaseDriver<boost::shared_future<cql::cql_future_result_t> > >(
        new CassandraDBDriver(dbServerHostName, dbServerPort));
    CassandraDBDriver::instanceExists = false;
  }
  return CassandraDBDriver::databaseDriverObject;
}
...
boost::shared_ptr<
    IDatabaseDriver<boost::shared_future<cql::cql_future_result_t> > >
CassandraDBDriver::getDatabaseDriverObject(const char *dbServerHostName,
                                           unsigned short int dbServerPort) {
  if (!CassandraDBDriver::instanceExists) {
    CassandraDBDriver::databaseDriverObject = boost::shared_ptr<
        IDatabaseDriver<boost::shared_future<cql::cql_future_result_t> > >(
