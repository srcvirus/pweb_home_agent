...
template <class KeyType, class ValueType> class LookupTable {
...
public:
  ...
  typename map<KeyType, ValueType>::iterator begin() { return table.begin(); }

  typename map<KeyType, ValueType>::iterator end() { return table.end(); }
};
...
template <typename KeyType, typename ValueType>
bool LookupTable<KeyType, ValueType>::update_strict(KeyType &key,
                                                    ValueType &value) {
  bool exists = false;
  pthread_rwlock_wrlock(&table_lock);
  if (table.find(key) == table.end()) {
