...
public:
	...
	long writeToBuffer()
	{
		...
		string endOfLables = "";
		offset = ProtocolHelper::placeStringIntoByteBuffer(this->buffer, offset, endOfLables);

		offset = ProtocolHelper::placeAtomicDataIntoByteBuffer(this->buffer, offset, htons((unsigned short)this->qType));
		offset = ProtocolHelper::placeAtomicDataIntoByteBuffer(this->buffer, offset, htons((unsigned short)this->qClass));
