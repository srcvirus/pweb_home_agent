...
public:
  ...
  void setLabels(const vector<string> &labels) { this->labels = labels; }

  QueryClass getClass() { return rClass; }

  void setClass(QueryClass clazz) { rClass = clazz; }

  std::vector<char> &getData() { return rData; }

  void setData(const std::vector<char> &data) { rData = data; }

  unsigned short getRdLength() { return rdLength; }

  void setRdLength(unsigned short rdLength) { this->rdLength = rdLength; }

  QueryType getType() { return rType; }

  void setType(QueryType type) { rType = type; }
  ...
  long writeToBuffer() {
    ...
    offset = ProtocolHelper::placeAtomicDataIntoByteBuffer(
        this->buffer, offset, htons((unsigned short)this->rType));
    offset = ProtocolHelper::placeAtomicDataIntoByteBuffer(
        this->buffer, offset, htons((unsigned short)this->rClass));
    offset = ProtocolHelper::placeAtomicDataIntoByteBuffer(this->buffer, offset,
                                                           ttl);
    offset = ProtocolHelper::placeAtomicDataIntoByteBuffer(
        this->buffer, offset, htons(this->rdLength));
    for (int i = 0; i < this->rdLength; i++)
      offset = ProtocolHelper::placeAtomicDataIntoByteBuffer(
          this->buffer, offset, this->rData[i]);
