...
request_handler::request_handler(const string &homeAgentAlias)
    : restapi(
          boost::shared_ptr<RESTAPIHelper>(new RESTAPIHelper(homeAgentAlias))),
...
void request_handler::handle_request(const request &req, reply &rep) {
  ...
  LOG(DEBUG) << "[Thread " << std::hex << tid << "] "
             << "http request path: " << request_path.c_str();
  ...
  if (qsp.get_value(methodKey, method_name)) {
    if (strtolower(method_name) == "is_username_available") {
      ...
      if (qsp.get_value("username", username) &&
        ...
        http_payload.append(
            restapi->registerUser(username, password, email, params));
        http_code = reply::ok;
      }
    } else if (strtolower(method_name) == "update_user") {
      string username, password, new_password, email, fullname, location,
          affiliation;
      if (qsp.get_value("username", username) &&
          qsp.get_value("password", password)) {
        boost::unordered_map<string, string> params;
      ...
      if (qsp.get_value("devicename", devicename) &&
        ...
        http_payload.append(
            restapi->isDevicenameAvailable(devicename, username));
        http_code = reply::ok;
      }
    } else if (strtolower(method_name) == "update_device") {
      string devicename, username, password, new_devicename, type, metadata, ip,
          port, dir_ip, dir_port, description, os, public_folder,
          private_folder, is_indexed;
      if (qsp.get_value("devicename", devicename) &&
          qsp.get_value("username", username) &&
          qsp.get_value("password", password)) {
        ...
        if (qsp.get_value("is_indexed", is_indexed)) {
          if (!boost::iequals(is_indexed, "true") &&
              !boost::iequals(is_indexed, "false"))
            return;
          params["is_indexed"] = is_indexed;
        }

        http_payload.append(
            restapi->updateDevice(devicename, username, password, params));
        http_code = reply::ok;
      }
      ...
      if (qsp.get_value("username", username) &&
        ...
        http_payload.append(
            restapi->deleteDevice(devicename, username, password));
        http_code = reply::ok;
      }
    } else if (strtolower(method_name) == "register_device") {
      string devicename, username, password, type, ip, port, dir_ip, dir_port,
          description, os, public_folder, private_folder, is_indexed;
      if (qsp.get_value("devicename", devicename) &&
          qsp.get_value("username", username) &&
          qsp.get_value("password", password) && qsp.get_value("ip", ip) &&
        ...
        http_payload.append(restapi->registerDevice(
            devicename, username, password, ip, port, is_indexed, params));
        http_code = reply::ok;
      }
...
bool request_handler::isValidIP(const string &input) {
  ...
  if (result != 4)
    return false;
  if (oct1 < 0 && oct1 > 255 && oct2 < 0 && oct2 > 255 && oct3 < 0 &&
      oct3 > 255 && oct4 < 0 && oct4 > 255)
    return false;
  return true;
}
