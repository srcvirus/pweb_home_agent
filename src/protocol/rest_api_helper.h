...
public:
  ...
  string updateUser(const string &username, const string &password,
                    boost::unordered_map<string, string> &params) {
    UserCassandraController userController(database);

    string errorCode = "";
    bool isAuthentic =
        userController.authenticateUser(username, password, errorCode);
    if (!errorCode.empty()) {
  ...
  string registerDevice(const string &devicename, const string &username,
                        ...
                        const string &port, const string &isIndexed,
                        boost::unordered_map<string, string> &params) {
    UserCassandraController userController(database);
    ...
    errorCode = "";
    isAvailable =
        deviceController.isDevicenameAvailable(devicename, username, errorCode);
    if (!errorCode.empty()) {
      return "{\"status\":\"error\", \"error\":\"CDB:" + errorCode + "\"}";
    }
    if (!isAvailable) {
    ...
    boost::shared_ptr<Device> device = boost::shared_ptr<Device>(new Device(
        username, devicename, ip, boost::lexical_cast<unsigned short>(port),
        (boost::iequals(isIndexed, "true")) ? true : false, timestamp, params));
  ...
  string isDevicenameAvailable(const string &devicename,
    ...
    for (int i = 0; i < REST_API_SUGGESION_COUNT; i++) {
      ...
      isAvailable =
          deviceController.isDevicenameAvailable(dname, username, errorCode);
      if (!errorCode.empty()) {
        return "{\"status\":\"error\", \"error\":\"CDB:" + errorCode + "\"}";
      }
      if (isAvailable) {
  ...
  string updateDevice(const string &devicename, const string &username,
                      const string &password,
                      boost::unordered_map<string, string> &params) {
    UserCassandraController userController(database);
  ...
  string getUserDevices(const string &username) {
    ...
    vector<boost::shared_ptr<Device> > deviceVector =
        deviceController.getUserDevices(username, errorCode);
    if (!errorCode.empty()) {
      return "{\"status\":\"error\", \"error\":\"CDB:" + errorCode + "\"}";
    }
    ...
    string result = "{\"status\":\"success\", \"username\":\"" + username +
                    "\", \"devices\":[";
    for (vector<boost::shared_ptr<Device> >::iterator it = deviceVector.begin();
         it != deviceVector.end(); ++it) {
      string is_indexed = "false";
      if ((*it)->isSearchable()) {
        is_indexed = "true";
      }
      result.append(
          "{\"devicename\":\"" + (*it)->getDevicename() + "\", \"type\":\"" +
          (*it)->getType() + "\", \"ip\":\"" + (*it)->getIp() +
          "\", \"port\":\"" + boost::lexical_cast<string>((*it)->getPort()) +
          "\", \"dir_ip\":\"" + (*it)->getDirIp() + "\", \"dir_port\":\"" +
          boost::lexical_cast<string>((*it)->getDirPort()) +
          "\", \"public_folder\":\"" + (*it)->getPublicFolder() +
          "\", \"private_folder\":\"" + (*it)->getPrivateFolder() +
          "\", \"os\":\"" + (*it)->getOs() + "\", \"description\":\"" +
          (*it)->getDescription() + "\", \"is_indexed\":" + is_indexed + "},");
      if (!dataAppended) {
        dataAppended = true;
      }
    }
  ...
  string getHomeAgentList() {
    ...
    vector<boost::shared_ptr<HomeAgentIndex> > homeAgents =
        homeAgentIndexController.getAllHomeAgentIndex();
    for (vector<boost::shared_ptr<HomeAgentIndex> >::iterator hit =
             homeAgents.begin();
         hit != homeAgents.end(); ++hit) {
      list += ("{\"alias\":\"" + (*hit)->getName() +
               ".pwebproject.net\", \"ip\":\"" + (*hit)->getIp() +
               "\", \"description\":\"" + (*hit)->getDescription() +
               "\", \"port\":\"" + port2String((*hit)->getPort()) + "\"},");
    }
    if (list.size() > 0) {
  ...
  string getDeviceList(const string &timestamp) {
    ...
    for (vector<boost::shared_ptr<HomeAgentIndex> >::iterator hit =
      ...
      result += "<home agent>";
      result +=
          "<hostname>" + (*hit)->getName() + ".pwebproject.net</hostname>";
    ...
    result += "<devices>";
    for (vector<boost::shared_ptr<User> >::iterator uit = users.begin();
         uit != users.end(); ++uit) {
      vector<boost::shared_ptr<Device> > devices =
          deviceController.getUserDevices((*uit)->getUsername(), errorCode);
      for (vector<boost::shared_ptr<Device> >::iterator dit = devices.begin();
           dit != devices.end(); ++dit) {
        if ((*dit)->getLastSeen() >= string2Timestamp(timestamp)) {
          result += "<device>";
          result.append("<owner>" + (*uit)->getFullname() + "</owner>");
          result.append("<name>" + (*dit)->getDevicename() + "." +
                        (*uit)->getUsername() + "</name>");
          result.append("<port>" + port2String((*dit)->getPort()) + "</port>");
          result.append("<type>" + (*dit)->getType() + "</type>");
          result.append("<timestamp>" +
                        timestamp2String((*dit)->getLastSeen()) +
                        "</timestamp>");
          result.append("<location>" + (*uit)->getLocation() + "</location>");
          result.append("<description>" + (*dit)->getDescription() +
                        "</description>");
          result += "</device>";
        }
        if ((*dit)->getContentTimestamp() >= string2Timestamp(timestamp)) {
          contentUpdates +=
              (*dit)->getDevicename() + "." + (*uit)->getUsername() + ",";
        }
      }
    }
    result += "</devices>";
  ...
  string getContentList(const string &devicename, const string &username) {
    ...
    boost::shared_ptr<Device> device =
        deviceController.getDevice(devicename, username, errorCode);
    if (!errorCode.empty()) {
      return "{\"status\":\"error\", \"error\":\"CDB:" + errorCode + "\"}";
    }
